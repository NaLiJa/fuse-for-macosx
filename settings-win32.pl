...
# settings-win32.pl: portable settings.c generator for MinGW/Win32 (from upstream Fuse).
# Used with FuseX settings.dat (7-column rows including optional funcn; skips nsarray).
...
CODE
    ...
    } elsif( $type eq 'null' or $type eq 'nsarray' ) {
...
CODE
    ...
    print "/*    *val_null = \&settings->$name; */\n" if( $options{$name}->{type} eq 'null' or $options{$name}->{type} eq 'nsarray' );
    print << "CODE";
    return 0;
  }
CODE
...
CODE
	...
	# No C field in portable settings_info
    }
}
